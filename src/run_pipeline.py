...
def run_all(train_path="train.csv", test_path="test.csv", target_col="LogP", outdir="outputs", smoke=False):
    ...
    for selector_name, features in selectors:
        ...
        # 1) Linear Regression (OLS backward elimination) - no scaling
        ...
        X_train_lr = X_train_corr[selected_ols].values
        X_test_lr = X_test_corr[selected_ols].values
        ...
        experiments.append({"selector": selector_name, "experiment": "baseline", "model": "Lasso_scaled", **metrics_lasso})
        pbar.update(1)
        ...
        experiments.append({"selector": selector_name, "experiment": "baseline", "model": "RandomForest", **metrics_rf})
        pbar.update(1)

        lgb = LGBMRegressor(random_state=42)
        metrics_lgb = train_and_eval(lgb, X_train_sel.values, X_test_sel.values, y_train, y_test)
        experiments.append({"selector": selector_name, "experiment": "baseline", "model": "LightGBM", **metrics_lgb})
        pbar.update(1)
        ...
        tuned_rf_record = {"selector": selector_name, "experiment": "optuna", "model": "RandomForest", **tuned_rf}
        ...
        tuned_lgb_record = {"selector": selector_name, "experiment": "optuna", "model": "LightGBM", **tuned_lgb}
